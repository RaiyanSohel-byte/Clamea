  ...
  return (
    <nav className="navbar sticky top-0 z-[100] bg-white/80 backdrop-blur-md border-b border-slate-100 px-4 md:px-8 lg:px-12">
      <div className="navbar-start">
        ...
        {/* Logo */}
        <a className="ubuntu text-2xl font-black tracking-tighter text-[#004C92] cursor-pointer hover:opacity-90 transition-opacity ml-2 lg:ml-0">
          Clamea
        </a>
      </div>
      ...
      <div className="navbar-end gap-2 md:gap-4">
        ...
        <div className="hidden sm:flex items-center bg-slate-100 rounded-lg p-0.5">
          <button
            onClick={() => setLang("en")}
            className={`px-3 py-1 text-[11px] font-bold rounded-md border transition-all ${lang === "en" ? "bg-white shadow-sm text-[#004C92]" : "text-slate-400 hover:text-slate-600"}`}
          >
            EN
          </button>
          <button
        ...
        {/* <select
          ...
          className="select select-ghost select-xs focus:bg-transparent font-bold text-slate-600 sm:hidden border border-[#004C92]"
        >
